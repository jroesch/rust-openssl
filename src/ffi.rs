...
// Initialization of the resources needed by FFI, such as error strings.
pub static mut VERIFY_IDX: c_int = -1;
pub static mut MUTEXES: *mut Vec<NativeMutex> = 0 as *mut Vec<NativeMutex>;
...
pub fn init() {
    ...
    unsafe {
        INIT.doit(|| {
            ...
            let verify_idx = SSL_CTX_get_ex_new_index(0, ptr::null(), None,
            None, None);
            assert!(verify_idx >= 0);
